...
通过-XX:G1MixedGCCountTarget指定一个周期内触发Mixed GC最大次数，默认值8。一次全局并发标记后，最多接着8次Mixed GC，把全局并发标记阶段生成的Cset里的Region拆分为最多8部分，然后在每轮Mixed GC里收集一部分。这个值要和上一个参数配合使用，8*10%=80%，应该来说会大于每次标记阶段的Cset集合了。
...
* 在需要修改 `G1MixedGCLiveThresholdPercent` 参数的时候,使用 `predict()` 方法根据历史数据预测未来区域的使用率。这个预测值可以作为新的 `G1MixedGCLiveThresholdPercent` 参数值
...
## G1MixedGCCountTarget调优

> 思路

* 主要关注`calc_min_old_cset_length()`
  ...
  * 在计算应该添加进CSet的最少老年region时，是将`G1MixedGCCountTarget`和1的最大值以及被标记了的老年代region进行相除得出对应的数量
  * 因此我们可以收集历史的`result`和`region_num`的数量放入`TruncatedSeq`序列之中，当收集到足够多的数据之后，就可以根据两个预测值计算出来的新的`G1MixedGCCountTarget`向上取整后和原本的`G1MixedGCCountTarget`相比较。
  * 如果不同，则说明此次的全局并发标记的region的数量上升或下降的幅度较大，因此，我们可以将预测的region数和预测的`result`进行相除，得出一个新的`G1MixedGCCountTarget`以确保每次GC回收的最少的老年代region数量能根据程序的运行状态、并发标记的效率来动态调整，保证MixedGC的效率。
...
* 环境准备
  ...
  * 随后声明对应的`TruncatedSeq`对象和`G1Predictions`对象
    ...
    ```c++
    TruncatedSeq _marking_result;
    TruncatedSeq _marking_candidates_regions;
    const G1Predictions * _predictor;
    ```
...
* 代码实现
  ...
  uint G1Policy::calc_min_old_cset_length(G1CollectionSetCandidates* candidates) const {
     g1_mixed_gc_count_target = (size_t)MAX2(G1MixedGCCountTarget,(unitx)1);
      ...
      _marking_candidates_regions.add((double)region_num);
      _marking_result.add((double)result);
    if(have_enough_data_for_prediction()) {
      const size_t predict_result = (size_t)predict(&_marking_result);
      const size_t predict_region_num = (size_t)predict(&_marking_candidates_regions);
      if(ceil(predict_region_num / predict_result) != g1_mixed_gc_count_target) {
        g1_mixed_gc_count_target = (size_t)MAX2(ceil(predict_region_num / predict_result),1);
      }
  ...
  bool G1Policy::have_enough_data_for_prediction() const {
    return ((size_t)_marking_candidates_regions.num() >= G1CollectedHeap::heap()->old_regions_count() / 2) &&
           ((size_t)_marking_result.num() >= G1AdaptiveIHOPNumInitialSamples);
  }
  ```
